...
namespace GraphicsProgramming
{
	class Lesson5 : Lesson
	{
		...
		public override void LoadContent(ContentManager Content, GraphicsDeviceManager graphics, SpriteBatch spriteBatch)
		{
			...
			GeneratePlane(4.0f, 500f);
			rt = new RenderTarget2D(graphics.GraphicsDevice, 
									graphics.PreferredBackBufferWidth, graphics.PreferredBackBufferHeight, false, graphics.PreferredBackBufferFormat, graphics.PreferredDepthStencilFormat);
		}

		private void GeneratePlane(float gridSize = 8.0f, float height = 8.0f)
		{
			// Get pixels
		...
		public override void Draw(GameTime gameTime, GraphicsDeviceManager graphics, SpriteBatch spriteBatch)
		{
			...
			Matrix Projection = Matrix.CreatePerspectiveFieldOfView((MathF.PI / 180f) * 65f, device.Viewport.AspectRatio, 0.01f, 3000f);
			...
			device.DepthStencilState = DepthStencilState.Default;
			device.RasterizerState = RasterizerState.CullNone;
			RenderModel(sphere, World);

			////Render Outside
			//device.RasterizerState = RasterizerState.CullCwounterClockwise;
			//RenderModel(sphere, World);

...
{
	class Lesson_2_Homework : Lesson
	{
		...
		private Vector4 lightCol;
		Vector3 LightPosition = Vector3.Right * 2 + Vector3.Up * 2 + Vector3.Backward * 2;

		public override void LoadContent(ContentManager Content, GraphicsDeviceManager graphics, SpriteBatch spriteBatch)
		{
			myEffect = Content.Load<Effect>("HomeWork2");
			crateTexture = Content.Load<Texture2D>("base");
			crateNormal = Content.Load<Texture2D>("NormalMap");
		}
		...
		public override void Draw(GameTime gameTime, GraphicsDeviceManager graphics, SpriteBatch spriteBatch)
		{
			...
			lightCol = new Vector4(MathF.Cos(time), Math.Max(MathF.Cos(time), 0.5f), MathF.Sin(time), 0);
			myEffect.Parameters["LightCol"].SetValue(lightCol);
